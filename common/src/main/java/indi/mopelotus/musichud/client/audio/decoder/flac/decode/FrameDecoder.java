...
// MusicHud TuneWeave adaptation; see META-INF/licenses/nayuki-flac/NOTICE.txt.
package indi.mopelotus.musichud.client.audio.decoder.flac.decode;

import java.io.IOException;
import java.util.Arrays;
import java.util.Objects;
import indi.mopelotus.musichud.client.audio.decoder.flac.common.FrameInfo;
...
public final class FrameDecoder {
    ...
    // A decorrelated 32-bit stereo side sample needs 33 signed bits.
    private long readSignedSample(int depth) throws IOException {
        if (depth <= 32) return in.readSignedInt(depth);
        if (depth != 33) throw new DataFormatException("Invalid sample depth");
        return ((long)in.readSignedInt(1) << 32) | Integer.toUnsignedLong(in.readUint(32));
    }
	...
	private void decodeSubframe(int sampleDepth, long[] result) throws IOException {
		...
		if (!(0 <= shift && shift <= sampleDepth))
			throw new AssertionError();
		sampleDepth -= shift;
		if (sampleDepth <= 0) throw new DataFormatException("No sample bits remain after wasted-bit encoding");

		// Read sample data based on type
		if (type == 0)  // Constant coding
			Arrays.fill(result, 0, currentBlockSize, readSignedSample(sampleDepth));
		else if (type == 1) {  // Verbatim coding
			for (int i = 0; i < currentBlockSize; i++)
				result[i] = readSignedSample(sampleDepth);
	...
	private void restoreLpc(long[] result, int[] coefs, int sampleDepth, int shift) {
		...
		if (shift < 0 || shift > 63)
			throw new IllegalArgumentException();
		long lowerBound = (-1L) << (sampleDepth - 1);
